...
def parse_args():
    ...
    parser.add_argument('--graph', default='CollegeMsg.pkl',
                        help='graph/network file')
    parser.add_argument('--label', default='data/cora/cora_node_label_dict.pkl',
                        help='node label file')
    parser.add_argument('--emb-file', default='C:/Users/hasee/PycharmProjects/o1/output',
...
def get_embedding_locations(path = "/usr/path/", startwith = "Zmatrix"):#C:\Users\hasee
    ...
    embedding_file_locations = ['']*len(all_files)
    for i in range(len(all_files)):
        raw_embedding_file_location = path+'/'+all_files[i]
        index = all_files[i][:-4]
        #print(index," ",raw_embedding_file_location)
        embedding_file_locations[int(index)] = raw_embedding_file_location

    if len(embedding_file_locations) == 0:
    ...
    for i in range(len(lines)):
        ...
        if True:
            ...
            for j in range(1,(len(items)-1)):
                embeddings.append(float(items[j]))
            final_dimension = float(items[len(items)-1][:-2])#to delete /n
            embeddings.append(final_dimension)
            #print(node_ID," ",embeddings)
            #if len(embeddings) != 40:
            #    print(embeddings, "embeddings not 40 dimension")
            #if node_ID not in sorted_node_union:
            #    print("node ID does not match,",node_ID)
