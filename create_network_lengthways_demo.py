...
def add_new_nodes_down_right_corner(graph, m, n, add_node_number = 4, offset = 0):
    """
    :param graph:
    :param pos:
    :param m:
...
def add_edges(graph, nodes):
    for i in range(len(nodes)):
        current_node = nodes[i]
        possible_neighbor = (current_node[0]+1, current_node[1]+1)
        if graph.has_node(possible_neighbor):
            graph.add_edge(current_node, possible_neighbor)
        possible_neighbor = (current_node[0]-1, current_node[1]-1)
        if graph.has_node(possible_neighbor):
            graph.add_edge(current_node, possible_neighbor)
        possible_neighbor = (current_node[0]+1, current_node[1]-1)
...
def create_demos(m, n, use_reservior = True, initial_frequency = False, combine_frequency = False, local_global_count = 2):
    ...
    g1 = add_new_nodes_down_right_corner(g0.copy(), m, n)
    g1 = add_new_edges_for_new_nodes(g1)
    nodes_add_edges = [(1,(n-2)),(3,(n-2)),(2,(n-2))]
    g1 = add_edges(g1.copy(), nodes_add_edges)
    ...
    g2 = add_new_nodes_down_right_corner(g1.copy(), m, n, offset=4)
    g2 = add_new_edges_for_new_nodes(g2)
    nodes_add_edges = [(1,(n-6)),(3,(n-6)),(2,(n-6))]
    g2 = add_edges(g2.copy(), nodes_add_edges)
    ...
    g3 = add_new_nodes_down_right_corner(g2.copy(), (m+1), n, add_node_number = 8)
    g3 = add_new_edges_for_new_nodes(g3)
    nodes_add_edges = [(4,(n-1)), (4, (n-3)),(4, (n-5))]
    g3 = add_edges(g3.copy(), nodes_add_edges)
    if use_reservior == False:
        reservior = {}
    for i in range(local_global_count+1):
        local_global_selected = i * 1 / local_global_count
...
def draw_selected_nodes(graph, selected_nodes, node_add, old_graph, graph_ID, parameter_number = 0):
    ...
    nx.draw_networkx_nodes(graph, graph_pos, node_color = node_inside_color_map, linewidths = 0.5, edgecolors= node_edge_color_map, node_size=8)
    nx.draw_networkx_edges(graph, graph_pos, edge_color= edge_color_map)
