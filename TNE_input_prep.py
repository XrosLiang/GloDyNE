...
def TNE_single_graph(nx_graph, input_name = '',graph_count = 0):
    if input_name != '':
        output_location = ''+input_name+'_TNE_prep/'
        input_name = input_name + '_'
    else:
        output_location = 'TNE_prep/'
    file_name = output_location + input_name+'TNE_input_graph'+str(graph_count)+'.txt'
...
def TNE_graph_line(index, nx_graph,all_nodes):
    ...
    line = str(index) + ',' + str(len(replaced_neighbors))
    for i in range(len(replaced_neighbors)):
        line += ':'+ str(int(replaced_neighbors[i]))+ ','+ '1'
    line += '\n'
    return line
...
def TNE_prep(path = 'AS733_dyn_graphs.pkl', input_name = '', add_all_nodes = True):
    ...
    print("start to generating TNE input")
    for i in range(len(nx_graphs)):
        print("generating input:",i+1, "out of ", len(nx_graphs))

        TNE_single_graph(nx_graphs[i], graph_count= i, input_name = input_name)

if __name__ == '__main__':
    TNE_prep(path = 'AS733_dyn_graphs.pkl', input_name= "AS_733")#input paht = 'balabala'
    """
    a = [1,2]
    b = [2,3]
    c = [4,5]
