...
def parse_args():
    ...
    # -------------------------------------------------method settings-----------------------------------------------------------
    parser.add_argument('--method', default='DynRWSG', choices=['DynRWSG', 'DynRWSG_random', 'DynRWSG_noacc', 'DeepWalk', 'GraRep', 'HOPE', 'DynRWSG_LB'],
...
def main(args):
    ...
    if args.method == 'DynRWSG':
        ...
        model = DynRWSG_random.DynRWSG_random(G_dynamic=G_dynamic, restart_prob=args.restart_prob, update_threshold=args.update_threshold, 
                                    ...
                                    window=args.window, workers=args.workers, negative=args.negative, seed=args.seed)
        model.sampling_traning()
    elif args.method == 'DynRWSG_LB':#have not add lower bound
        from libne import DynRWSG_lower_bound
        model = DynRWSG_lower_bound.DynRWSG(G_dynamic=G_dynamic, restart_prob=args.restart_prob, update_threshold=args.update_threshold,
                                    emb_dim=args.emb_dim, num_walks=args.num_walks, walk_length=args.walk_length,
                                    window=args.window, workers=args.workers, negative=args.negative, seed=args.seed)
        model.sampling_traning()
