...
class DynRWSG_random(object):
    ...
    def sampling_traning(self):
        ...
        w2v = gensim.models.Word2Vec(sentences=None, size=self.emb_dim, window=self.window, sg=1, hs=0,
                                     negative=self.negative, ns_exponent=0.75,
                                     ...
                                     trim_rule=None)  # w2v constructor, default parameters
        ...
        for t in range(len(self.G_dynamic)):
            ...
            if t == 0:  # offline ----------------------------
                ...
                sentences = simulate_walks(nx_graph=G0, num_walks=self.num_walks, walk_length=self.walk_length,
                                           restart_prob=None)  # restart_prob=None or 0 --> deepwalk
                ...
                node_update_list, self.reservoir = most_affected_nodes(graph_t0=G0, graph_t1=G1,
                                                                       update_threshold=self.update_threshold,
                                                                       reservoir_dict=self.reservoir)  # note: self.reservoir for accumulated changes
                """
                node_list = list(G1.nodes)
                print(f'G1 graph size: {len(list(G1.nodes))}')
                if len(node_update_list) < len(list(G1.nodes))*self.lower_bound:
                    add_node_number = int(len(list(G1.nodes))*self.lower_bound - len(node_update_list))
                    newly_added_nodes = random.sample(node_list, add_node_number)
                    for node_index in range(len(newly_added_nodes)):
                        if newly_added_nodes[node_index] in self.reservoir:
                            del self.reservoir[newly_added_nodes[node_index]]
                    print(f'randomly added: {len(newly_added_nodes)}')
                    node_update_list = list(set(node_update_list).union(set(newly_added_nodes)))
                    print(f'total update: {len(node_update_list)}')

                """
                print('final node count:',len(node_update_list))
                sentences = simulate_walks(nx_graph=G1, num_walks=self.num_walks, walk_length=self.walk_length,
                                           restart_prob=self.restart_prob, affected_nodes=node_update_list)
                sentences = [[str(j) for j in i] for i in sentences]
...
def most_affected_nodes(graph_t0, graph_t1, update_threshold, reservoir_dict):
    ...
    num_random_nodes = int(len(list(G1.nodes)) * 0.1)
